...
// TOKEN returns nth token from s using characters in sep as trim boundaries or null string
	...
	// check for the existence of animated door sets (0 = either side, 1 = left side, 2 = right side)
		...
		for (n = 0; n < vehicles.size(); n++) {
			...
			bool left = vehicles[n].HasMesh("left-passenger-door");
			bool right = vehicles[n].HasMesh("right-passenger-door");
	...
	// check for the existence of animated door sets (0 = either side, 1 = left side, 2 = right side)
		...
		for (n = 0; n < vehicles.size(); n++) {
			bool facing = vehicles[n].GetDirectionRelativeToTrain();
			bool left = vehicles[n].HasMesh("left-door");
			bool right = vehicles[n].HasMesh("right-door");
			if ((facing and left) or (!facing and right)) hasLeft = true;
			if ((facing and right) or (!facing and left)) hasRight = true;
			switch (side) {
				case 0:		if (hasLeft and hasRight) return true;
...
// WEATHERMONITOR sends a message to rolling stock using the library whenever it starts or stops raining
		...
		{
			// check for rain
			if ((raining and World.GetWeatherType() < 2) or (!raining and World.GetWeatherType() > 1)) 
			{
...
// NUMBERING //////////////////////////////////////////////////////////////////////////////////////////////////
	...
	{
		Asset regNumbers = Caller.GetAsset().FindAsset("numbers");
...
// DOOR CONTROL /////////////////////////////////////////////////////////////////////////////////////////////
	...
	// modes: 0=close 1=open left 2=open right
		...
		for(n = 0; n < vehicles.size(); n++) {
			switch(mode) {
				case 0: 	vehicles[n].SetDoorAnimationState("left-passenger-door",false);
									vehicles[n].SetDoorAnimationState("right-passenger-door",false);
									break;
				case 1:		if (vehicles[n].GetDirectionRelativeToTrain()) {
										vehicles[n].SetDoorAnimationState("left-passenger-door",true);
									} else {
										vehicles[n].SetDoorAnimationState("right-passenger-door",true);
									}
									break;
	...
	// modes: 0=close 1=open left 2=open right
		...
		for(n = 0; n < vehicles.size(); n++) {
			switch(mode) {
				case 0: 	vehicles[n].SetMeshAnimationState("left-door",false);
									vehicles[n].SetMeshAnimationState("right-door",false);
									break;
				case 1:		if (vehicles[n].GetDirectionRelativeToTrain()) {
										vehicles[n].SetMeshAnimationState("left-door",true);
									} else {
										vehicles[n].SetMeshAnimationState("right-door",true);
									}
									break;
				case 2:		if (vehicles[n].GetDirectionRelativeToTrain()) {
										vehicles[n].SetMeshAnimationState("right-door",true);
...
// Object Property Interface Calls
		...
		if (function == "GetPropertyName") {
			...
			else if (pID == "onemesh") {result = U.UserString("Select_Attachments");}
			else if (pID == "twomesh") {result = U.UserString("Select_siattachments");}
			else if (pID == "threemesh") {result = U.UserString("Select_poattachments");}
			else if (pID == "fourmesh") {result = U.UserString("Select_laattachments");}
		...
		if (function == "GetPropertyValue") {
			string result;
			string pID = Strings[1];
			if (pID == "registration") result = SSSoup.GetNamedTag("vehicleNumber");
			else if (pID == "registrationg") result = SSSoup.GetNamedTag("vehicleNumber");
			else result = Strings[0];
			return result;
		}
		...
		if (function == "GetPropertyType") {
			...
			else if (	pID== "registration" or pID== "registrationg") result = "string";
	...
	//	Interface.Print("HTML:electromanager="+electromanager);	
			return
				U.Header(textured or textured2 or textured3 or textured4 or attached or attached2 or attached3 or attached4 or attached5,-1,U.UserString("OPTIONS"),"imgadd")
			...
			+	U.Table(U.Section == -1 and attached,
					"<tr>"	+ U.Cell(U.W1,U.UserString("Sub_Meshes"))
			...
			+	U.Table(U.Section == -1 and attached3,
					"<tr>"	+ U.Cell(U.W1,U.UserString("Po_Meshes"))
			...
			+	U.Table(U.Section == -1 and attached4,
					"<tr>"	+ U.Cell(U.W1,U.UserString("La_Meshes"))
