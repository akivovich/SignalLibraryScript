...
class ZmvBase isclass ZmvInterface
{
    ...
    bool m_bDebug;
    bool m_bSemiAutomatProp;    //is Signal is semiautomat
...
/*			
            ...
            content = content + HTMLWindow.MakeCell(HTMLWindow.MakeLink(allLink, "<font color=#cede20>"+mST.GetString("signal-for-all")+"</font>"),"bgcolor=#999999");
*/			
        }
...
//		if (m_bDebug) Print("ShowAutoblocLenses", "m_bAutoblockCurrent=" + m_bAutoblockCurrent);		
    ...
    void showLenses()
    {
		...
		if (m_lenseTypes[m_nLensesState] or m_nLensesState == ZmvSignalTypes.PS)
        {
			...
			string[] lenses = null;
			if (lensesData)
				lenses = m_lenseTypes[nLensesState].getLenses();			
    ...
    {        
        ...
        if (nextObject == null or nNewLensesState == ZmvSignalTypes.R or m_speedLimits[nNewLensesState] == 0)
        {
    ...
    //if (m_bDebug or IsDebug()) Print("getNextObject", "nextMarker="+ m_nextMarker.GetName());
                }
            }
			else if (nextObject.isclass(Signal))
			{
                if (thesearch.GetFacingRelativeToSearchDirection())
                {
    //if (m_bDebug or IsDebug()) Print("getNextObject", "nextSignal="+ (cast<Signal>(nextObject)).GetName());
                    break;
                }
			}
			else if (nextObject.isclass(Vehicle))
            {
    //if (m_bDebug or IsDebug()) Print("getNextObject", "nextVehicle="+ (cast<Vehicle>(nextObject)).GetName());
                m_DistanceToVehicle = (int)thesearch.GetDistance();
				break;
            }
			else
			{
    //if (m_bDebug or IsDebug()) Print(" ", "nextObject="+ (cast<GameObject>(nextObject)).GetName());
			}
			nextObject = thesearch.SearchNext();
		}
    ...
    //Lenses State operations ===============================================================================================
    int getNewLensesStatePS(object nextObject)
    {
        if (m_bDebug) Print("getNewLensesStatePS", "m_nLensesState =" + m_nLensesState);
	    if (nextObject != null)
        {
            if (nextObject.isclass(Vehicle) and nextObject != m_nextObject and m_DistanceToVehicle < 20)
			{
                Vehicle v = cast<Vehicle>(nextObject);
				
	//Print("getNewLensesStatePS", m_nextTrain, ","+v.GetMyTrain().GetFrontmostLocomotive().GetName());

				if (v.GetMyTrain() != m_nextTrain)
					return ZmvSignalTypes.R;
			}
        }
        return ZmvSignalTypes.PS; 
    }
    ...
    {
        if (m_bDebug) Print("getNewLensesStateBySignal", "nPrevSignalState =" + nPrevSignalState);
        return ZmvSignalTypes.R;
    }
...
//if (IsDebug()) Print("getNewLensesState(object nextObject)","");
		...
		if (nextObject != null)
		{
			...
			if (!m_bNextIsVehicle)                
			{
//if (m_bDebug or IsDebug()) Print("$$getNewLensesState$$","nextObject.isclass(ZmvSignalInterface)="+(string)nextObject.isclass(ZmvSignalInterface)+",name="+(cast<GameObject>(nextObject)).GetName());
				
				if (nextObject.isclass(ZmvSignalInterface))
				{
					ZmvSignalInterface signal = cast<ZmvSignalInterface>(nextObject);
					int state = signal.GetLensesState();
//if (IsDebug()) Print("$$getNewLensesState$$","name="+signal.GetName()+",state="+state+",m_nextSpeedLimitForALS="+signal.GetSpeedLimit()/KPH_TO_MPS);
					...
					{
						if (state < 0)  nNewLensesState = getNewLensesState(ZmvSignalTypes.R);
						else			nNewLensesState = getNewLensesState(state);
					}
					if (m_bDebug /*or IsDebug()*/) Print("$$getNewLensesState$$","name="+signal.GetName()+",state="+signal.GetLensesState());
					m_nextTrain = null;
				...
				else if (nextObject.isclass(Signal))
				{
					...
					nNewLensesState = getNewLensesStateBySignal(signal.GetSignalState());
					m_nextTrain = null;
					m_nextSpeedLimitForALS = signal.GetSpeedLimit()/KPH_TO_MPS;
				}				
			}
			...
			{
				if (m_bDebug /*or IsDebug()*/) Print("$$getNewLensesState$$","NextObject-Vehicle");
				if ((m_bSemiAutomat and !m_bSemiAutomatCurrent) or (!m_bAutoblock and m_bAutoblockCurrent))
				{
					Vehicle cur = cast<Vehicle>(nextObject);
					Train train = cur.GetMyTrain();
					Vehicle front = train.GetFrontmostLocomotive();
		
		//Print("getNewLensesState", m_nextTrain, ","+front.GetName());

					if (train != m_nextTrain)
					{
						if (train.GetVehicles().size() == 1 or cur != front)
						{
			//Print("$$getNewLensesState$$","m_bAutoblock="+m_bAutoblock+",m_bAutoblockCurrent="+m_bAutoblockCurrent);
        ...
        if (m_bDebug /*or IsDebug()*/) Print("getNewLensesState","NewLensesState="+(string)nNewLensesState);
	...
	//if (IsDebug()) Print("getNewFinalLensesState","m_nLensesState="+m_nLensesState);
        if (m_nLensesState == ZmvSignalTypes.PS)
        {
            nNewLensesState = getNewLensesStatePS(nextObject);
        }
        else if (m_nextMarker != null and m_nextMarker.IsClosed())
        {
        ...
        {
            nNewLensesState = getNewLensesState(nextObject);        
        }
...
if (m_bDebug /*or IsDebug()*/) Print("ApplyUpdatedState","old="+m_nLensesState+"+ new="+ nNewLensesState);
		...
		if (nNewLensesState != m_nLensesState)
        {
            ...
            setCurrentState();
//            if (m_prevSignal)
//                m_signal.PostMessage(m_prevSignal,"ZmvPreviousSignalChanged","", 0);
        }
...
//        m_lenseTypes = new ZmvLensesData[ZmvSignalTypes.MAX];
        ...
        {        
            lenseCur = new ZmvLensesData();
            lenseCur.addLense(ZmvLenseTypes.scR);
            lenseCur.addLense(ZmvLenseTypes.scWf);
            m_lenseTypes[ZmvSignalTypes.PS] = lenseCur;
            m_allLenses.addLense(ZmvLenseTypes.scWf);
            if (m_bDebug) Print("InitLenseTypes","ZmvSignalTypes.PS, m_allLenses.getLenses().size()="+m_allLenses.getLenses().size());
        }
...
/*
    void updateNextSignal()
    {
        ZmvSignalInterface nextSignal = getNextSignal(false);

        if (nextSignal != null)
            m_signal.PostMessage(nextSignal, "ZmvInit", "", 0);
    }
*/	
	...
	string GetCurrentState(StringTable ST)
	{
		if (m_nLensesState == ZmvSignalTypes.PS)
		{
			return ST.GetString("signal-state-wf");
		}
...
//Print("SetAutoblock:m_bAutoblockCurrent","true");				
	...
	void SetInvitationManually(bool set)
	{
		...
		if (set)
		{
			...
			m_nLensesState = ZmvSignalTypes.PS;
			m_nextTrain = getNextTrain();
	//Print("TurnOnInvitationSignal", m_nextTrain, "");
		}
		else
		{
			if (m_bSemiAutomat)
...
//if (IsDebug()) Print("GetLastAlsValue=",m_alsValue);
	...
	//if (IsDebug()) Print("SetNextAlsValue-0:", "speedLimit="+speedLimit);	
				if (speedLimit == 0)
				{
					if (nLensesState == ZmvSignalTypes.R or nLensesState == ZmvSignalTypes.PS) 
					{
						als = ZmvAls.ALS_0;
					}
	...
	//if (IsDebug()) Print("speedLimit == 0", "als="+als+",als_next="+als_next);					
				...
				{					
					if (nLensesState == ZmvSignalTypes.PS)
					{
						als = ZmvAls.ALS_OC;
						als_next = -1;
	//if (IsDebug()) Print("PS (PS)", "speedLimit="+speedLimit+",als="+als+",als_next="+als_next);
					}
